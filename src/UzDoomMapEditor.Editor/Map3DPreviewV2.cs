...
public sealed class Map3DPreview : Control
{
    ...
    public void ResetView()
    {
        ...
        else
        {
            ...
            var minH = _project.Sectors.Count == 0 ? 0 : _project.Sectors.Min(s => Math.Min(s.FloorHeight, s.RampEndHeight));
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        foreach (var face in BuildFaces())
            if (TryProjectFace(face, camera, out var result))
    ...
    private List<Face> BuildFaces()
    {
        ...
        foreach (var sector in _project.Sectors)
        {
            ...
            var floorBase = sector.FloorShape == SectorFloorShape.Ramp ? Color.FromArgb(82, 118, 92) : Color.FromArgb(86, 103, 112);
            var floorColor = Shade(floorBase, light * 0.82f);
            var ceilingColor = Shade(Color.FromArgb(105, 112, 126), light * 0.72f);
            var wallColor = Shade(Color.FromArgb(112, 125, 135), light);

            faces.Add(new Face(
                sector.Vertices.Select(v => new Vector3(v.X, (float)sector.GetFloorHeightAt(v.X, v.Y), v.Y)).ToArray(),
                floorColor,
            ...
            for (var i = 0; i < sector.Vertices.Count; i++)
            {
                ...
                var floorA = (float)sector.GetFloorHeightAt(a.X, a.Y);
                var floorB = (float)sector.GetFloorHeightAt(b.X, b.Y);
                faces.Add(new Face(
                    new[]
                    {
                        new Vector3(a.X, floorA, a.Y),
                        new Vector3(b.X, floorB, b.Y),
        ...
        foreach (var door in _project.Doors)
        {
            ...
            for (var i = 0; i < vertices.Count; i++)
            {
                ...
                faces.Add(new Face(
                    ...
                    }, fill, Color.FromArgb(120, 70, 25)));
            }
            faces.Add(new Face(vertices.Select(v => new Vector3(v.X, top, v.Y)).ToArray(), Shade(fill, 1.15f), Color.FromArgb(120, 70, 25)));
        }
    ...
    private void DrawThings(Graphics g, Camera camera)
    {
        foreach (var thing in _project.Things)
        {
            ...
            g.DrawLine(pen, a, b);
            g.FillEllipse(brush, b.X - 4, b.Y - 4, 8, 8);
        }
    }

    private float FindFloorHeight(int x, int y)
    {
        foreach (var sector in _project.Sectors)
            if (GeometryUtil.PointInPolygon(x, y, sector.Vertices))
                return (float)sector.GetFloorHeightAt(x, y);
        return 0;
    }
    ...
    private void DrawOverlay(Graphics g)
    {
        ...
        const string help = "LMB orbit   MMB/RMB pan   Wheel zoom   •   ramps/stairs preview live";
    ...
    private Camera GetCamera()
    {
        ...
        var offset = new Vector3(cosPitch * MathF.Cos(_yaw), MathF.Sin(_pitch), cosPitch * MathF.Sin(_yaw)) * _distance;
    ...
    private bool TryProject(Vector3 point, Camera camera, out PointF screen, out float depth)
    {
        ...
        screen = new PointF(ClientSize.Width / 2f + x * scale, ClientSize.Height / 2f - y * scale);
        return true;
    }

    private static Color Shade(Color source, float factor)
        => Color.FromArgb(source.A, Math.Clamp((int)(source.R * factor), 0, 255), Math.Clamp((int)(source.G * factor), 0, 255), Math.Clamp((int)(source.B * factor), 0, 255));
    ...
    protected override void OnMouseMove(MouseEventArgs e)
    {
        ...
        else if (_panning)
        {
            ...
            _target = _targetStart + horizontal + flatForward * (dy * scale);
            Invalidate();
        }
    }
