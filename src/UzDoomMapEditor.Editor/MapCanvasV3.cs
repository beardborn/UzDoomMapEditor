...
{
    [Browsable(false)] public Sector Sector { get; }
    [Browsable(false)] public int Index { get; }
    ...
    [Category("Vertex"), ReadOnly(true)] public string SectorName => Sector.Name;
    [Category("Vertex"), ReadOnly(true)] public int VertexNumber => Index + 1;
...
{
    [Browsable(false)] public Sector Sector { get; }
    [Browsable(false)] public int StartIndex { get; }
    ...
    [Category("Edge"), ReadOnly(true)] public string SectorName => Sector.Name;
    [Category("Edge"), ReadOnly(true)] public int EdgeNumber => StartIndex + 1;
    [Category("Edge"), ReadOnly(true)] public double Length => Math.Round(Math.Sqrt(Math.Pow(B.X - A.X, 2) + Math.Pow(B.Y - A.Y, 2)), 2);
    [Category("Start"), ReadOnly(true)] public int X1 => A.X;
    [Category("Start"), ReadOnly(true)] public int Y1 => A.Y;
    [Category("End"), ReadOnly(true)] public int X2 => B.X;
    [Category("End"), ReadOnly(true)] public int Y2 => B.Y;
...
{
    private sealed record SharedWall(
        Sector SectorA,
        int EdgeA,
        Sector SectorB,
        int EdgeB,
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        if (_tool == EditorTool.Door && _hoverWall is not null && !_drawingArea)
            DrawDoorWallHint(e.Graphics, _hoverWall);
    ...
    private void DrawGrid(Graphics g)
    {
        ...
        var count = 0;
        for (var x = firstX; x <= maxX + _gridSize && count < 3000; x += _gridSize, count++)
        {
        ...
        count = 0;
        for (var y = firstY; y <= maxY + _gridSize && count < 3000; y += _gridSize, count++)
        {
    ...
    private void DrawSector(Graphics g, Sector sector)
    {
        ...
        var baseFill = sector.FloorShape == SectorFloorShape.Ramp
            ? Color.FromArgb(62, 94, 122, 82)
            : sector.Name.Contains("Step", StringComparison.OrdinalIgnoreCase)
                ? Color.FromArgb(62, 122, 98, 72)
                : Color.FromArgb(48, 92, 112, 128);
        using var fill = new SolidBrush(selected ? Color.FromArgb(78, 52, 122, 172) : baseFill);
        ...
        var suffix = sector.FloorShape == SectorFloorShape.Ramp
            ? $"  {sector.FloorHeight}→{sector.RampEndHeight}"
            : string.Empty;
        g.DrawString(sector.Name + suffix, Font, textBrush, screenCenter.X + 5, screenCenter.Y + 5);

        if (sector.FloorShape == SectorFloorShape.Ramp)
            DrawRampArrow(g, sector);
        if (_tool is EditorTool.Vertex or EditorTool.Edge || selected)
            DrawSectorHandles(g, sector);
    }

    private void DrawRampArrow(Graphics g, Sector sector)
    {
        var center = SectorCenter(sector);
        ...
        PointF start;
        PointF end;
        switch (sector.RampDirection)
        {
            case RampDirection.East:
                start = WorldToScreen(minX, center.Y);
                end = WorldToScreen(maxX, center.Y);
                break;
            case RampDirection.West:
                start = WorldToScreen(maxX, center.Y);
                end = WorldToScreen(minX, center.Y);
                break;
            case RampDirection.North:
                start = WorldToScreen(center.X, minY);
                end = WorldToScreen(center.X, maxY);
                break;
            default:
                start = WorldToScreen(center.X, maxY);
                end = WorldToScreen(center.X, minY);
                break;
        }

        using var pen = new Pen(Color.LightGreen, 2f) { EndCap = LineCap.ArrowAnchor };
    ...
    private void DrawDoor(Graphics g, Door door, bool selected)
    {
        ...
        using var fill = new SolidBrush(selected ? Color.FromArgb(175, 255, 165, 35) : Color.FromArgb(120, 220, 125, 25));
        ...
        g.DrawString($"{door.Name} {door.Width}×{door.Depth}", Font, textBrush, rect.X + 4, rect.Y + 4);
    }

    private void DrawDoorWallHint(Graphics g, SharedWall wall)
    {
        PointF a;
        PointF b;
        if (wall.Horizontal)
        {
            a = WorldToScreen(wall.Start, wall.Constant);
            b = WorldToScreen(wall.End, wall.Constant);
        }
        else
        {
            a = WorldToScreen(wall.Constant, wall.Start);
            b = WorldToScreen(wall.Constant, wall.End);
        }

        using var pen = new Pen(Color.Gold, 4f) { DashStyle = DashStyle.Dash };
        g.DrawLine(pen, a, b);
    }
    ...
    private void DrawDraftArea(Graphics g)
    {
        ...
        var color = _drawingTool switch
        {
            EditorTool.Door => Color.Orange,
            EditorTool.Ramp => Color.LightGreen,
            EditorTool.Stairs => Color.Khaki,
            _ => Color.Gold
        };
        using var pen = new Pen(color, 2f) { DashStyle = DashStyle.Dash };
        g.DrawRectangle(pen, rect.X, rect.Y, rect.Width, rect.Height);
    }
    ...
    protected override void OnMouseDown(MouseEventArgs e)
    {
        ...
        switch (_tool)
        {
            ...
            case EditorTool.Ramp:
            case EditorTool.Stairs:
                BeginAreaDraw(_tool, world);
                break;
            ...
            case EditorTool.Vertex:
                ...
                else Select(HitSector(e.Location));
                break;
    ...
    private void BeginAreaDraw(EditorTool tool, PointF world)
    {
        _drawingArea = true;
        _drawingTool = tool;
        _areaStart = world;
    ...
    protected override void OnMouseMove(MouseEventArgs e)
    {
        ...
        if (_panning)
        {
            _origin = new PointF(_panOriginStart.X + e.X - _panMouseStart.X, _panOriginStart.Y + e.Y - _panMouseStart.Y);
            Invalidate();
            return;
        }

        if (_tool == EditorTool.Door && !_drawingArea && !_dragging)
        {
            _hoverWall = FindSharedWallNear(e.Location);
            Invalidate();
        }
        ...
        if (_drawingArea)
        {
            ...
            if (width >= _gridSize && depth >= _gridSize)
            {
                switch (_drawingTool)
                {
                    ...
                    case EditorTool.Ramp:
                    {
                        var ramp = RampStairDesigner.CreateRamp($"Ramp {_project.Sectors.Count + 1}", x, y, width, depth, _areaStart, _areaCurrent);
                        RampStairDesigner.MatchRampToNeighbours(ramp, _project.Sectors);
                        _project.Sectors.Add(ramp);
                        Select(ramp);
                        break;
                    }
                    case EditorTool.Stairs:
                    {
                        var stairs = RampStairDesigner.CreateStairs(
                            $"Stairs {_project.Sectors.Count + 1}", x, y, width, depth, _areaStart, _areaCurrent,
                            (name, sx, sy, sw, sd) => Sector.Rectangle(name, sx, sy, sw, sd));
                        RampStairDesigner.MatchStairsToNeighbours(stairs, _project.Sectors);
                        _project.Sectors.AddRange(stairs);
                        Select(stairs.Count > 0 ? stairs[^1] : null);
                        break;
                    }
                    case EditorTool.Door:
                    {
    ...
    private bool TryPlaceWallDoor(Point screenPoint)
    {
        var wall = FindSharedWallNear(screenPoint);
        if (wall is null) return false;
        var raw = ScreenToWorld(screenPoint);
        var span = wall.End - wall.Start;
        if (span < 32) return false;

        var doorWidth = Math.Min(RampStairDesigner.DefaultDoorWidth, span);
        var halfDoor = doorWidth / 2;
        var centerAlong = wall.Horizontal ? raw.X : raw.Y;
        centerAlong = Math.Clamp(centerAlong, wall.Start + halfDoor, wall.End - halfDoor);
        var snappedCenter = SnapTo((int)Math.Round(centerAlong), 8);
        snappedCenter = Math.Clamp(snappedCenter, wall.Start + halfDoor, wall.End - halfDoor);

        var halfThickness = RampStairDesigner.DefaultDoorDepth / 2;
        RecessSharedWall(wall.SectorA, wall.EdgeA, wall, halfThickness);
        RecessSharedWall(wall.SectorB, wall.EdgeB, wall, halfThickness);

        var nextTag = _project.Doors.Count == 0 ? 100 : _project.Doors.Max(d => d.Tag) + 1;
        var floor = Math.Max(wall.SectorA.FloorHeight, wall.SectorB.FloorHeight);
        Door door;
        if (wall.Horizontal)
        {
            door = new Door
            {
                Name = $"Door {_project.Doors.Count + 1}",
                X = snappedCenter - halfDoor,
                Y = wall.Constant - halfThickness,
                Width = doorWidth,
                Depth = RampStairDesigner.DefaultDoorDepth,
                FloorHeight = floor,
                Tag = nextTag,
                SideTexture = wall.SectorA.WallTexture,
                FloorTexture = wall.SectorA.FloorTexture,
                CeilingTexture = wall.SectorA.CeilingTexture,
                LightLevel = Math.Min(wall.SectorA.LightLevel, wall.SectorB.LightLevel)
            };
        }
        else
        {
            ...
            {
                Name = $"Door {_project.Doors.Count + 1}",
                X = wall.Constant - halfThickness,
                Y = snappedCenter - halfDoor,
                Width = RampStairDesigner.DefaultDoorDepth,
                Depth = doorWidth,
                FloorHeight = floor,
                Tag = nextTag,
                SideTexture = wall.SectorA.WallTexture,
    ...
    private static void RecessSharedWall(Sector sector, int edgeIndex, SharedWall wall, int inset)
    {
        if (sector.Vertices.Count < 3 || edgeIndex < 0 || edgeIndex >= sector.Vertices.Count) return;
        var a = sector.Vertices[edgeIndex];
        var b = sector.Vertices[(edgeIndex + 1) % sector.Vertices.Count];
        var center = SectorCenter(sector);
        MapVertex p1;
        MapVertex p2;
        MapVertex i1;
        ...
        if (wall.Horizontal)
        {
            var first = a.X <= b.X ? wall.Start : wall.End;
            var second = a.X <= b.X ? wall.End : wall.Start;
        ...
        {
            var first = a.Y <= b.Y ? wall.Start : wall.End;
            var second = a.Y <= b.Y ? wall.End : wall.Start;
        ...
        for (var i = 0; i < sector.Vertices.Count; i++)
        {
            var current = sector.Vertices[i];
            AddUnique(rebuilt, new MapVertex(current.X, current.Y));
            if (i != edgeIndex) continue;
            if (!SamePoint(current, p1)) AddUnique(rebuilt, p1);
            AddUnique(rebuilt, i1);
            AddUnique(rebuilt, i2);
            var next = sector.Vertices[(i + 1) % sector.Vertices.Count];
            if (!SamePoint(next, p2)) AddUnique(rebuilt, p2);
        }

        if (rebuilt.Count > 1 && SamePoint(rebuilt[0], rebuilt[^1]))
            rebuilt.RemoveAt(rebuilt.Count - 1);
        sector.Vertices = rebuilt;
    }

    private SharedWall? FindSharedWallNear(Point screenPoint)
    {
        var world = ScreenToWorld(screenPoint);
        var tolerance = Math.Max(4.0, 12.0 / Math.Max(0.05, _zoom));
        SharedWall? best = null;
        var bestDistance = double.MaxValue;

        for (var aIndex = 0; aIndex < _project.Sectors.Count; aIndex++)
        {
            var sectorA = _project.Sectors[aIndex];
            for (var bIndex = aIndex + 1; bIndex < _project.Sectors.Count; bIndex++)
            {
                var sectorB = _project.Sectors[bIndex];
                var centerA = SectorCenter(sectorA);
                var centerB = SectorCenter(sectorB);

                for (var ea = 0; ea < sectorA.Vertices.Count; ea++)
                {
                    var a1 = sectorA.Vertices[ea];
                    var a2 = sectorA.Vertices[(ea + 1) % sectorA.Vertices.Count];
                    var aHorizontal = a1.Y == a2.Y;
                    var aVertical = a1.X == a2.X;
                    if (!aHorizontal && !aVertical) continue;

                    for (var eb = 0; eb < sectorB.Vertices.Count; eb++)
                    {
                        var b1 = sectorB.Vertices[eb];
                        var b2 = sectorB.Vertices[(eb + 1) % sectorB.Vertices.Count];
                        if (aHorizontal && b1.Y == b2.Y && a1.Y == b1.Y)
                        {
                            if ((centerA.Y - a1.Y) * (centerB.Y - a1.Y) >= 0) continue;
                            var start = Math.Max(Math.Min(a1.X, a2.X), Math.Min(b1.X, b2.X));
                            var end = Math.Min(Math.Max(a1.X, a2.X), Math.Max(b1.X, b2.X));
                            if (end - start < 32) continue;
                            var distance = DistanceToAxisSegment(world.X, world.Y, true, a1.Y, start, end);
                            if (distance <= tolerance && distance < bestDistance)
                            {
                                bestDistance = distance;
                                best = new SharedWall(sectorA, ea, sectorB, eb, true, a1.Y, start, end);
                            }
                        }
                        else if (aVertical && b1.X == b2.X && a1.X == b1.X)
                        {
                            if ((centerA.X - a1.X) * (centerB.X - a1.X) >= 0) continue;
                            var start = Math.Max(Math.Min(a1.Y, a2.Y), Math.Min(b1.Y, b2.Y));
                            var end = Math.Min(Math.Max(a1.Y, a2.Y), Math.Max(b1.Y, b2.Y));
                            if (end - start < 32) continue;
                            var distance = DistanceToAxisSegment(world.X, world.Y, false, a1.X, start, end);
                            if (distance <= tolerance && distance < bestDistance)
                            {
                                bestDistance = distance;
                                best = new SharedWall(sectorA, ea, sectorB, eb, false, a1.X, start, end);
                            }
                        }
                    }
                }
    ...
    private static double DistanceToAxisSegment(float x, float y, bool horizontal, int constant, int start, int end)
    {
        if (horizontal)
        {
            var px = Math.Clamp(x, start, end);
            var dx = x - px;
            var dy = y - constant;
            return Math.Sqrt(dx * dx + dy * dy);
        }
        var py = Math.Clamp(y, start, end);
        var vx = x - constant;
        var vy = y - py;
        return Math.Sqrt(vx * vx + vy * vy);
    }

    private static void AddUnique(List<MapVertex> vertices, MapVertex point)
    {
        if (vertices.Count == 0 || !SamePoint(vertices[^1], point))
            vertices.Add(new MapVertex(point.X, point.Y));
    }

    private static bool SamePoint(MapVertex a, MapVertex b) => a.X == b.X && a.Y == b.Y;
    private static int SnapTo(int value, int grid) => (int)Math.Round(value / (double)grid) * grid;
    ...
    private object? HitTest(Point screenPoint)
    {
        for (var i = _project.Things.Count - 1; i >= 0; i--)
        {
            ...
            if (dx * dx + dy * dy <= 18f * 18f) return thing;
        }
    ...
    private Sector? HitSector(Point screenPoint)
    {
        ...
        for (var i = _project.Sectors.Count - 1; i >= 0; i--)
        {
            ...
            if (GeometryUtil.PointInPolygon(world.X, world.Y, sector.Vertices)) return sector;
        }
        return null;
    }
    ...
    private PointF ScreenToWorld(Point point) => new((point.X - _origin.X) / _zoom, -((point.Y - _origin.Y) / _zoom));
    ...
    private static PointF SectorCenter(Sector sector)
    {
        ...
        return new PointF((float)sector.Vertices.Average(v => v.X), (float)sector.Vertices.Average(v => v.Y));
    }
}
